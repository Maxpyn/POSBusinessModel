...
class Tenant(models.Model):
    ...
    financial_kpi_pin_hash = models.CharField(max_length=128, blank=True, default="")
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return self.name

    def set_financial_kpi_pin(self, pin):
        self.financial_kpi_pin_hash = make_password(pin)
        self.save(update_fields=["financial_kpi_pin_hash"])

    def check_financial_kpi_pin(self, pin):
        return bool(self.financial_kpi_pin_hash) and check_password(
            pin,
            self.financial_kpi_pin_hash,
        )
...
class SuspendedOrder(models.Model):
    class Status(models.TextChoices):
        HELD = "held", "Held"

    tenant = models.ForeignKey(Tenant, on_delete=models.CASCADE, related_name="suspended_orders")
    ...
    def __str__(self):
        return self.label or f"Held order #{self.pk}"


class SuspendedOrderItem(models.Model):
    order = models.ForeignKey(SuspendedOrder, on_delete=models.CASCADE, related_name="items")
